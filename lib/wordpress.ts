...
function getUrl(path: string, query?: Record<string, any>) {
    ...
    return `${normalizedBaseUrl}${path}${params ? `?${params}` : ""}`;
}
...
// Add this interface for media sizes
...
// Add this new function to get featured media details
export async function getFeaturedMediaDetails(mediaId: number): Promise<MediaResponse> {
    if (!mediaId) return null;

    const url = getUrl(`/wp-json/wp/v2/media/${mediaId}`);
    return wordpressFetch<MediaResponse>(url, {
        next: {
            ...defaultFetchOptions.next,
            tags: ["wordpress", `media-${mediaId}`],
        },
    });
}